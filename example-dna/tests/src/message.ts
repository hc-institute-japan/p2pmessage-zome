...
const config = Config.gen({
  ...
  bobby: Config.dna('../example-dna.dna.gz', null),
  carly: Config.dna('../example-dna.dna.gz', null)
})

function send_message(message) {
    return (conductor, caller) =>
      conductor.call(caller, "message", "send_message", message);
};
  
function receive_message() {
  return (conductor, caller) =>
    conductor.call(caller, "message", "receive_message", null);
};

function get_all_messages() {
  return (conductor, caller) =>
    conductor.call(caller, "message", "get_all_messages", null);
}

function get_all_messages_from_addresses(agentlist) {
  return (conductor, caller) =>
    conductor.call(caller, "message", "get_all_messages_from_addresses", agentlist);
}

function get_batch_messages_on_conversation(messagerange) {
  return (conductor, caller) => 
  conductor.call(caller, "message", "get_batch_messages_on_conversation", messagerange);
}
